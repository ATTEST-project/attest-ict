...
public class LoadProfile implements Serializable {
    ...
    @OneToMany(mappedBy = "loadProfile", cascade = { CascadeType.REMOVE, CascadeType.REFRESH }, orphanRemoval = true)
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    ...
    public LoadProfile season(String season) {
    ...
    public LoadProfile typicalDay(String typicalDay) {
    ...
    public LoadProfile mode(Integer mode) {
        this.setMode(mode);
        return this;
    }
    ...
    public LoadProfile timeInterval(Double timeInterval) {
    ...
    public LoadProfile uploadDateTime(Instant uploadDateTime) {
    ...
    public LoadProfile inputFile(InputFile inputFile) {
    ...
    public void setLoadElVals(Set<LoadElVal> loadElVals) {
        if (this.loadElVals != null) {
            this.loadElVals.forEach(i -> i.setLoadProfile(null));
        }
        if (loadElVals != null) {
            loadElVals.forEach(i -> i.setLoadProfile(this));
        }
        this.loadElVals = loadElVals;
    }

    public LoadProfile loadElVals(Set<LoadElVal> loadElVals) {
        this.setLoadElVals(loadElVals);
        return this;
    }

    public LoadProfile addLoadElVal(LoadElVal loadElVal) {
        this.loadElVals.add(loadElVal);
        loadElVal.setLoadProfile(this);
        return this;
    }

    public LoadProfile removeLoadElVal(LoadElVal loadElVal) {
        this.loadElVals.remove(loadElVal);
        loadElVal.setLoadProfile(null);
        return this;
    }
    ...
    public LoadProfile network(Network network) {
    ...
    public boolean equals(Object o) {
        ...
        return id != null && id.equals(((LoadProfile) o).id);
    }
