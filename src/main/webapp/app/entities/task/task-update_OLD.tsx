...
export const TaskUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const toolLogFilesLoading = useAppSelector(state => state.toolLogFile.loading);
  const simulationLoading = useAppSelector(state => state.simulation.loading);
  const toolsLoading = useAppSelector(state => state.tool.loading);
  ...
  const saveEntity = values => {
    ...
    console.log('Task saveEntity: values: ', values);

    /* eslint-disable-next-line no-console */
    console.log('Task saveEntity: entity: ', taskEntity);
    ...
    /* eslint-disable-next-line no-console */
    console.log('Task EDIT: entity: ', entity);
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          toolLogFile: taskEntity?.toolLogFileId,
          simulation: taskEntity?.simulationUuid,
          tool: taskEntity?.tool?.id,
          user: taskEntity?.user?.id,
        };

  /* eslint-disable-next-line no-console */
  console.log('Task EDIT: defaultValues: ', defaultValues());
  return (
    <div>
      ...
      <Row className="justify-content-center">
        <Col md="8">
          {taskLoading || toolLogFilesLoading || simulationLoading || toolsLoading ? (
            <p>Loading...</p>
          ) : (
