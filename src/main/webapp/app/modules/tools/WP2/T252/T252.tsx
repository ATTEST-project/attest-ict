...
import Config from 'app/modules/tools/WP2/T252/config/config';
import { runT252Tool, reset as retry } from 'app/modules/tools/WP2/T252/reducer/tool-execution.reducer';
import { downloadResults } from 'app/modules/tools/WP2/T252/reducer/tool-results.reducer';
...
const T252 = (props: any) => {
  ...
  const isRunning = useAppSelector(state => state.t252ToolExecution.loading);
  const response = useAppSelector(state => state.t252ToolExecution.entity);
  const completed = useAppSelector(state => state.t252ToolExecution.updateSuccess);
  ...
  const parametersToSend = (parameters: any) => {
    const { input_network_path, input_resources_path, input_other_path, input_da_bids, input_agc_signal } = parameters;
    return {
      ...
      input_da_bids: input_da_bids?.[0].name,
      input_agc_signal: input_agc_signal?.[0].name,
    };
  };
  ...
  const submitMethod = data => {
    ...
    const finalForm = {
      ...
      toolName: TOOLS_INFO.T25_REAL_TIME_TOOL.name,
      files: [...Object.values(parameters).map(file => file?.[0])],
