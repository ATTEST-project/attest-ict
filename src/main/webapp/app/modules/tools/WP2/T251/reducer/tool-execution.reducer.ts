...
export const runT251Tool = createAsyncThunk(
  't251_tool/run',
...
export const T251ToolExecutionSlice = createEntitySlice({
  name: 't251_tool_run',
  initialState,
  extraReducers(builder) {
    builder
      .addMatcher(isPending(runT251Tool), state => {
        state.loading = true;
        state.updating = true;
      })
      .addMatcher(isFulfilled(runT251Tool), (state, action) => {
...
export const { reset } = T251ToolExecutionSlice.actions;

export default T251ToolExecutionSlice.reducer;
